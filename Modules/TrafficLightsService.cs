...
public class TrafficLightsService
{
    ...
    private string BuildTicketSectionHtml(TrafficTicketData ticket)
    {
        ...
        sb.Append($"<div class='info-row'><span class='info-label'>Date:</span> {(ticket.DateTimeOpened.HasValue ? ToPacificTime(ticket.DateTimeOpened.Value).ToString("M/d/yyyy h:mm:sstt") : "")}</div>");
        ...
        sb.Append($"<div class='info-row'><span class='info-label'>Start Date/Time:</span> {ticket.StartDate?.ToShortDateString() ?? ""} {System.Net.WebUtility.HtmlEncode(ticket.StartTime)}</div>");
    ...
    // QuickBase timestamps parse into server-local time (UTC on Cloud Run); Unspecified kinds are treated as UTC.
    private static DateTime ToPacificTime(DateTime value)
    {
        return value.Kind == DateTimeKind.Unspecified
            ? TimeZoneInfo.ConvertTimeFromUtc(DateTime.SpecifyKind(value, DateTimeKind.Utc), PacificTimeZone)
